  ...
  class MtdSorteos
  {
    ...
    public static string CuponEmitidoLeyenda(string Cuil)
    {
      using (var context = new lts_sindicatoDataContext())
      {
        ...
        if (emitido.Count() > 0)
        {
          return "EL CUPON   Nº " + emitido.SingleOrDefault().event_cupon_nro + "   YA FUE EMITIDO PARA ESTE SOCIO EL DIA   " + emitido.SingleOrDefault().event_cupon_fecha + "   POR EL   USUARIO: '' " + MtdUsuarios.GetUsuario(Convert.ToInt32(emitido.SingleOrDefault().UsuarioId)) + " ''    DESEA REIMPRMIR EL CUPON  ?????";
        }
        else
        {
          return "";
